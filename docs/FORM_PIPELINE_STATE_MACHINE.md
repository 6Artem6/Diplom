...
### Объединение OCR-блоков (merge)

Объединение выполняется **в два этапа**:

1. **В S2 (сразу после получения блоков от OCR)** — предварительное объединение по геометрии:
   - Критерий: **высота соседних блоков ±25%** (0.75 ≤ h_a/h_b ≤ 1.25).
   - По вертикали **не объединяем**: только одна линия (center_y в пределах ~0.5× median_line_height).
   - Горизонтальный зазор: объединять, если зазор ≤ max(80 px, 0.8× min(w_a, w_b)).
   - Сортировка по (x, y), объединение слева направо.

2. **После S3 (когда строки уже найдены)** — объединение по строкам:
   - Для каждой строки взять OCR-блоки, чей center_y попадает в [row.y_min, row.y_max].
   - Внутри строки отсортировать блоки по x; объединять соседние с **высотой ±25%** и малым горизонтальным зазором (как выше).
   - В итоге фраза в одной строке (например «Size Scale Test») даёт один OCR-блок, а не три.

Использование: после S3 объединённые по строкам блоки подставляются в строки (RowElement с ocr_block); S4 и дальше работают уже с объединённым текстом.
